...
class DPOTrainer:
    ...
    #modified
    def __perform_dpo_or_ipo_step(self, input_ids_w, attention_mask_w, input_ids_l, attention_mask_l, return_stats, ln_ches: Optional[torch.Tensor] = None):
        ...
        #modified
            # Compute per-example DPO or IPO loss (no reduction yet)
        if self.config.objective == "dpo":
            # per-example DPO: -logsigmoid(kl * (pi_logratio - ref_logratio))
            pi_logratio = seq_logprobs_w - seq_logprobs_l     # shape [B]
            ref_logratio = seq_old_logprobs_w - seq_old_logprobs_l  # shape [B]
            pi_logratio = pi_logratio.clamp(min=-10.0, max=10.0)
            per_example_loss = -F.logsigmoid(self.config.kl_coeff * (pi_logratio - ref_logratio))
        else:
            # IPO per-example squared loss
            pi_logratio = seq_logprobs_w - seq_logprobs_l
            ref_logratio = seq_old_logprobs_w - seq_old_logprobs_l
            per_example_loss = (pi_logratio - ref_logratio - 1 / (2 * self.config.kl_coeff)) ** 2

        # Apply ln_ches-based weighting if provided and configured
        if ln_ches is not None and getattr(self.config, "ches_alpha", 0.0) is not None and self.config.ches_alpha > 0.0:
            # ensure ln_ches is float tensor on the same device and matches batch dim
            ln_ches = ln_ches.float().to(seq_logprobs_w.device)
            if ln_ches.ndim == 0:
                ln_ches = ln_ches.unsqueeze(0).expand(per_example_loss.shape[0])
            # compute linear weight and clamp to >= 0
            # Update in dpo_trainer (1).py
            weights = (1.0 - self.config.ches_alpha * ln_ches).clamp(min=0.0, max=1.0)
            loss = (weights * per_example_loss).mean()
        else:
            loss = per_example_loss.mean()

        # add sft coeff term unchanged (not weighted)
        if self.config.sft_coeff > 0:
            loss = loss + -self.config.sft_coeff * seq_logprobs_w.mean()
        #modified
        ...
        if return_stats:
            ...
            if ln_ches is not None and getattr(self.config, "ches_alpha", 0.0) > 0.0:
                stats["train/mean_ln_ches"] = ln_ches.mean().detach().cpu().item()
                stats["train/mean_ches_weight"] = weights.mean().detach().cpu().item()
    ...
    def _step(
            ...
            ln_ches: Optional[torch.Tensor] = None, #modified
    ):
        if self.config.objective == "cross_entropy":
            return self.__perform_cross_entropy_step(input_ids_w, attention_mask_w, return_stats)
        elif self.config.objective in ["dpo", "ipo"]:
            return self.__perform_dpo_or_ipo_step(input_ids_w, attention_mask_w, input_ids_l, attention_mask_l, return_stats, ln_ches) #modified
        else:
            raise ValueError(f"Objective {self.config.objective} not supported")
