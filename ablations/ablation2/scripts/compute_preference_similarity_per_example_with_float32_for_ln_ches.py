...
def get_and_log_hidden_embedding_based_pref_similarities(logger, dataset, model, device, tokenizer):
    ...
    for example in tqdm(dataset):
        ...
        # 1. Cast embeddings to float32 first for precision
        preferred_hidden_embed_f32 = preferred_hidden_embed.to(torch.float32)
        dispreferred_hidden_embed_f32 = dispreferred_hidden_embed.to(torch.float32)

        # 2. Perform summation in float32
        sum_preferred_embeddings = preferred_hidden_embed_f32.sum(dim=0)
        sum_dispreferred_embeddings = dispreferred_hidden_embed_f32.sum(dim=0)

        # 3. Calculate CHES score in float32
        ...
        # 4. Calculate Length-Normalized CHES in float32
    ...
    logger.info(f"\n------------------------------------------------------------------------------------------------------------------------------\n"
                ...
                f"25th percentile: {torch.quantile(ches_scores.float(), q=0.25)} , "
                f"Median: {ches_scores.median()} , "
                f"75th percentile: {torch.quantile(ches_scores.float(), q=0.75)} , "
    ...
    logger.info(f"\n------------------------------------------------------------------------------------------------------------------------------\n"
                ...
                f"25th percentile: {torch.quantile(ln_ches_scores.float(), q=0.25)} , "
                f"Median: {ln_ches_scores.median()} , "
                f"75th percentile: {torch.quantile(ln_ches_scores.float(), q=0.75)} , "
    ...
    logger.info(f"\n------------------------------------------------------------------------------------------------------------------------------\n"
                ...
                f"25th percentile: {torch.quantile(last_hidden_embedding_inner_prods.float(), q=0.25)} , "
                f"Median: {last_hidden_embedding_inner_prods.median()} , "
                f"75th percentile: {torch.quantile(last_hidden_embedding_inner_prods.float(), q=0.75)} , "
