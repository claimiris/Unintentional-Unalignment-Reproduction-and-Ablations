...
flags.DEFINE_float("ches_alpha", 0.0, "alpha multiplier for ln_ches weighting in DPO loss (0.0 = no weighting)")
...
def __train_process_pref_batch(pref_batch, tokenizer, trainer, use_orig_rejected: bool = False):
    ...
    #modified
    if "ln_ches" in pref_batch:
        ln_ches = pref_batch["ln_ches"]
    else:
        ln_ches = [0.0] * len(chosen)

    return input_ids_w, attention_mask_w, input_ids_l, attention_mask_l, ln_ches
...
def __prepare_datasets():
    ...
    #modification
    if FLAGS.preference_similarity_path:
        pref_info = torch.load(FLAGS.preference_similarity_path, map_location="cpu")
        # pull ln_ches if available; fall back to raw ches
        if "ln_ches_scores" in pref_info:
            pref_vals = pref_info["ln_ches_scores"]
        elif "ches_scores" in pref_info:
            # fallback but prefer length-normalized; will normalize later if needed
            pref_vals = pref_info["ches_scores"]
        else:
            raise ValueError("preference_similarity file does not contain ln_ches_scores or ches_scores fields")

        sample_indices = pref_info["sample_indices"]  # indices into the original dataset used when computing CHES
        # Create a full-length array for the dataset defaulting to 0.0
        ln_ches_full = [0.0] * len(train_dataset)
        # Map pref_vals (which correspond to sample_indices ordering) into full list
        for local_idx, global_idx in enumerate(sample_indices):
            # some safety: only assign if index in range
            if 0 <= global_idx < len(ln_ches_full):
                ln_ches_full[global_idx] = float(pref_vals[local_idx])

        # add field 'ln_ches' to every example by index
        def add_ln_ches_scores(example, idx):
            return {"ln_ches": ln_ches_full[idx]}

        train_dataset = train_dataset.map(add_ln_ches_scores, with_indices=True)
        test_dataset = test_dataset.map(add_ln_ches_scores, with_indices=True)
    # --- end block ---
    ...
    for epoch in tqdm(range(FLAGS.num_train_epochs), desc="Epochs"):
        ...
        for batch_index, pref_batch in tqdm(enumerate(trainer.dataloader), desc="Train batches", total=len(trainer.dataloader)):
            with trainer.accelerator.accumulate(trainer.model):
                input_ids_w, attention_mask_w, input_ids_l, attention_mask_l, ln_ches = __train_process_pref_batch(pref_batch, trainer.tokenizer, trainer)
                ln_ches_tensor = torch.tensor(ln_ches, dtype=torch.float32, device=trainer.accelerator.device)
                train_stats = trainer.step(input_ids_w, attention_mask_w, input_ids_l, attention_mask_l, ln_ches=ln_ches_tensor)
