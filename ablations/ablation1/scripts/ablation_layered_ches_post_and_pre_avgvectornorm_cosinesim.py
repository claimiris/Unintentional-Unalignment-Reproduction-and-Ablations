...
def get_and_log_layer_wise_metrics(logger, dataset, model, device, tokenizer):
    ...
    for example in tqdm(dataset):
        ...
        # --- [OPTION 1] NORMALIZE BY AVERAGE NORM (REPLACING LENGTH) ---
        for i in range(num_layers):
            ...
            # 1. Calculate Vector Norms (The new "Lengths")
            norm_p = torch.norm(h_p_sum)
            norm_m = torch.norm(h_m_sum)
            
            # The Scaling Factor: Squared Average Norm
            # We square it because CHES (Dot Product) is quadratic (Magnitude * Magnitude)
            avg_norm = (norm_p + norm_m) / 2.0
            scaling_factor = avg_norm ** 2
            
            if scaling_factor < 1e-9: scaling_factor = 1.0

            # 2. Compute Raw CHES
            # (Remember: CHES = Good*Bad - Good^2)
            raw_ches = torch.dot(h_p_sum, h_m_sum) - (norm_p ** 2)
            
            # 3. Apply Normalization
            # We assign this to BOTH metrics to replace the length-based versions
            norm_ches = raw_ches / scaling_factor
            
            layer_ches_storage_post[i].append(norm_ches.cpu())    # Originally Raw
            layer_ln_ches_storage_post[i].append(norm_ches.cpu()) # Originally Length-Normed

            # 4. Last Token Inner Product (Normalized by Norms = Cosine Similarity)
            norm_p_last = torch.norm(h_p_last)
            norm_m_last = torch.norm(h_m_last)
            if norm_p_last < 1e-9 or norm_m_last < 1e-9:
                inner_post = torch.tensor(0.0)
            else:
                inner_post = torch.dot(h_p_last, h_m_last) / (norm_p_last * norm_m_last)
        ...
        for i in range(1,num_layers):
            ...
            norm_p = torch.norm(w_pre_sum)
            norm_m = torch.norm(l_pre_sum)
            
            # 2. Calculate Scaling Factor (Average Norm Squared)
            avg_norm = (norm_p + norm_m) / 2.0
            scaling_factor = avg_norm ** 2
            if scaling_factor < 1e-9: scaling_factor = 1.0

            # 3. Calculate Normalized CHES (Replacing standard CHES)
            raw_ches_pre = torch.dot(w_pre_sum, l_pre_sum) - (norm_p ** 2)
            norm_ches_pre = raw_ches_pre / scaling_factor
            
            layer_ches_storage_pre[i].append(norm_ches_pre.cpu())

            # 4. Calculate LN-CHES (Replacing length-norm with Avg Norm)
            # We store the same normalized value here to maintain the "Unit-less" property
            layer_ln_ches_storage_pre[i].append(norm_ches_pre.cpu())

            # 5. Last Token Inner Product (Normalized -> Cosine Similarity)
            last_p = w_resp_pre[-1]
            last_m = l_resp_pre[-1]
            norm_last_p = torch.norm(last_p)
            norm_last_m = torch.norm(last_m)
            
            if norm_last_p < 1e-9 or norm_last_m < 1e-9:
                inner_pre = torch.tensor(0.0)
            else:
                inner_pre = torch.dot(last_p, last_m) / (norm_last_p * norm_last_m)
